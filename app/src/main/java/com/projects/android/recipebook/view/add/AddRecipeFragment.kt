		...
		binding.apply {
			...
			activity?.onBackPressedDispatcher?.addCallback(viewLifecycleOwner, true) {
				if (addRecipeViewModel.checkRecipe(binding, _bindingIngredientsList)) {//if there are no errors
					AlertDialog.Builder(requireContext()).setTitle("Подтвердите выход?").setIcon(R.drawable.ic_baseline_dangerous_24)
						.setMessage("Рецепт будет удален").setPositiveButton(android.R.string.ok) { _, _ ->
